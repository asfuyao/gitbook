...
--检查游标状态
SELECT ( CASE
           WHEN CURSOR_STATUS('global', 'c_Dev') = 1 THEN '游标的结果集至少有一行'
           WHEN CURSOR_STATUS('global', 'c_Dev') = 0 THEN '游标的结果集为空'
           WHEN CURSOR_STATUS('global', 'c_Dev') = -1 THEN '游标被关闭'
           WHEN CURSOR_STATUS('global', 'c_Dev') = -2 THEN '游标不适用'
           WHEN CURSOR_STATUS('global', 'c_Dev') = -3 THEN '游标不存在'
         END ) AS RESULT;

--根据状态释放游标
IF( SELECT CURSOR_STATUS('global', 'c_Dev')) > -3
BEGIN
  DEALLOCATE DD;
END;
```
