...
```sql
-- 查询当前数据库所有表空间占用，按总空间降序排列
SELECT
    t.NAME AS 表名,
    s.Name AS 架构名,
    p.rows AS 数据行数,
    -- 总空间（数据+索引）
    CAST(ROUND(((SUM(a.total_pages) * 8) / 1024.00 / 1024.00), 2) AS NUMERIC(18, 2)) AS 总空间GB,
    -- 纯数据空间
    CAST(ROUND(((SUM(a.used_pages) * 8) / 1024.00 / 1024.00), 2) AS NUMERIC(18, 2)) AS 已用空间GB,
    -- 未使用空间
    CAST(ROUND(((SUM(a.total_pages) - SUM(a.used_pages)) * 8) / 1024.00 / 1024.00, 2) AS NUMERIC(18, 2)) AS 未用空间GB
FROM 
    sys.tables t
INNER JOIN 
